...
flattenUnits l = mapM flattenUnit l >>= return . concat

flattenUnit :: Unit -> IO [Unit]
flattenUnit u@(Unit _ _ _) = return [u]
flattenUnit (Include (Atom txt) Nothing) = withFile (unpack txt) ReadMode act
    where
      act = join . liftM flattenUnits . liftM readTPTP . Data.Text.IO.hGetContents
...
translateFormula s v (Negated formula) = translateFormula s v formula >>= return . tNeg s
